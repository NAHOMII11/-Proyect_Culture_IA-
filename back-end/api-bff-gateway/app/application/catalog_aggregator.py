...
- Llama servicios en paralelo con asyncio.gather
- Modo degradado: si analytics o geo fallan, el catálogo igual responde
"""
...
async def get_catalog_with_scores() -> dict:
    """
    GET /catalog
    Agrega: Place Service + Analytics (score por lugar)
    Si analytics no responde, devuelve el catálogo igualmente (degraded mode).
    """
    ...
    async def _enrich_score(place: dict) -> dict:
        try:
            ...
            # Analytics caído o place sin score aún → no bloqueamos el catálogo
            place["score"] = None
...
async def get_place_detail(place_id: str) -> dict:
    ...
    Agrega: Place Service + Geo Service + Analytics
    Usa return_exceptions=True → si un servicio cae, los demás igual responden.
    """
    ...
    # Place es obligatorio; si falla, propagamos el error
    if isinstance(place_result, Exception):
        raise place_result

    response = dict(place_result)

    # Geo: opcional, se agrega si está disponible
    ...
    # Score: opcional, se agrega si está disponible
...
async def get_ranking_with_names() -> dict:
    """
    GET /ranking
    Obtiene el ranking de analytics y enriquece con nombre del lugar desde Place Service.
    Si Place Service falla en algún lugar puntual, se omite el nombre.
    """
