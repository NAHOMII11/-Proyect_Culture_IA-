...
def send_place_to_api(payload: dict) -> tuple[bool, str, str | None]:
    try:
        response = httpx.post(
        ...
        return False, f"Places API respondió {response.status_code}: {response.text}", None

    except Exception as e:
        return False, str(e), None

def send_geo_point(place_id: str, latitude: float, longitude: float, address: str) -> None:
    try:
        httpx.post(
            "http://cultureia-geo-api:8002/geo/points",
            json={
                "place_id": place_id,
                "latitude": latitude,
                "longitude": longitude,
...
def validate_csv(batch_id: str, content: str) -> None:
    ...
    try:
        ...
        for i, row in enumerate(reader, start=2):
            ...
            ok, error_message, place_id = send_place_to_api(payload)

            if ok:
                valid += 1
                # Send geo point if we have coordinates
                if place_id and payload.get("latitude") and payload.get("longitude"):
                    send_geo_point(place_id, payload["latitude"], payload["longitude"], payload.get("address", ""))
            else:
                invalid += 1
        ...
        send_audit_event(
            event_type="importacion_procesada",
