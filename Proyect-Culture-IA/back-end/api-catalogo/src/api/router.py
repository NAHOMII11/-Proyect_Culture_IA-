...
from ..infrastructure.audit_client import send_audit_event
...
def create_place(place: PlaceCreate, db: Session = Depends(get_db)):
    ...
    created = repo.create_place(place)
    place_id = created.get("id") if isinstance(created, dict) else getattr(created, "id", None)
    if place_id:
        send_audit_event(
            event_type="lugar_creado",
            source_service="places-service",
            reference_id=str(place_id),
            payload_summary={
                "name": created.get("name") if isinstance(created, dict) else getattr(created, "name", ""),
                "category": created.get("category") if isinstance(created, dict) else getattr(created, "category", ""),
            },
        )
    return created
