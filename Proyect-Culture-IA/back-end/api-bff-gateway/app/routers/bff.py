...
async def dashboard_summary():
    auth_url = f"{UPSTREAM_SERVICES['v1_auth']}/auth/health"
    ...
    iaenri_url = f"{UPSTREAM_SERVICES['v1_iaenri']}/v1_iaenri/health"
    analytics_url = f"{UPSTREAM_SERVICES['v1_analytics']}/v1_analytics/health"
    audit_url = f"{UPSTREAM_SERVICES['v1_audit']}/v1_audit/health"
    ...
    async with get_async_client() as client:
        ...
        analytics_resp = await client.get(analytics_url)
    ...
    return {
        ...
        "profile4": analytics_resp.json() if analytics_resp.content else {},
        "profile5": audit_resp.json() if audit_resp.content else {},
    }
...
async def get_nearby_places(
    ...
    radius_km = min(max(radius_km, 0.5), 100.0)
    ...
    async with get_async_client() as client:
        for item in geo_results:
            ...
            name, category, address, imagelink, description = None, None, None, None, None
