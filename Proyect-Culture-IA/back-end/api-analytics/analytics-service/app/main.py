...
import httpx
from .core.audit_client import send_audit_event
...
async def get_ranking():
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            def _in_colombia(place: dict) -> bool:
                lat = place.get("latitude")
                lng = place.get("longitude")
                if lat is None or lng is None:
                    return False
                return -4.3 <= float(lat) <= 13.5 and -81.8 <= float(lng) <= -66.8

            unique_places = [p for p in unique_places if _in_colombia(p)]
...
async def score_all_places(db: Session = Depends(get_db)):
    ...
    2. Calcula variables desde datos reales del catalogo
    ...
    try:
        async with httpx.AsyncClient() as client:
            ...
            for place in places:
                ...
                try:
                    importance = float(place.get("importance_score") or 0.5)
                    tags = place.get("tags") or []
                    tag_bonus = min(len(tags) * 0.05, 0.2) if isinstance(tags, list) else 0
                    has_geo = place.get("latitude") is not None and place.get("longitude") is not None

                    variables = {
                        "data_quality": round(min(importance + tag_bonus, 1.0), 2),
                        "cultural_relevance": round(min(importance, 1.0), 2),
                        "accessibility": 0.7 if has_geo else 0.3,
                    }
            ...
            send_audit_event(
                event_type="puntuacion_calculada",
                source_service="analytics-service",
                reference_id=f"puntuacion-{processed_count}",
                payload_summary={
