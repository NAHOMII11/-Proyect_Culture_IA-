...
function UploadCsvPage() {
  ...
  const handleGenerateCoords = async () => {
    try {
      ...
      const geoResponse = await fetch('http://localhost:8000/api/v1_geo/geo/asignar')
      ...
      for (let i = 0; i < 2; i++) {
        try {
          const enrichResponse = await fetch('http://localhost:8000/api/v1_iaenri/enrichments/enriquecer', {
            method: 'GET',
      ...
      try {
        const analyticsResponse = await fetch('http://localhost:8000/api/v1_analytics/analytics/puntuar', {
          method: 'GET',
          headers: { 'Content-Type': 'application/json' }
        })
