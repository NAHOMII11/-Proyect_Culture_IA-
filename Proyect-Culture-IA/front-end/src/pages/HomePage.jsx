...
function HomePage() {
  ...
  const { highlightPlaces } = useMapPlaces()
  ...
  useEffect(() => {
    const loadPlaces = async () => {
      try {
        ...
        const activePlaces = data.filter((place) => place.status === 'active').map((place) => ({
          ...place,
      ...
      <main className="page">
        ...
        <section className="section" id="map-section">
          <div className="section-head">
            ...
            <p>
              ...
              {highlightPlaces.length > 0 && (
                <> · <strong>{highlightPlaces.length}</strong> del asistente IA</>
              )}
            </p>
          </div>
          ...
          <TouristMap
            ...
            places={filteredPlaces}
            highlightPlaces={highlightPlaces}
          />
        </section>
        {/* Dashboard KPIs debajo del mapa */}
